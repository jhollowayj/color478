...
caffe.set_mode_cpu() #Uncommented JACOB
...
def main():
  ...
  if run_caffe:
    ...
    print "Done caffe session" #JACOB
  ...
  if run_tf:
    ...
    print "createing model from caffe"
    m = ModelFromCaffe()
    print "building images on model from caffe"
    m.build(images)

    print "..." #Starting session #JACOB
    tf.Session()
    print "Check!"
    with tf.Session() as sess:
      print ".run next"
      sess.run(tf.initialize_all_variables())
      print "finished.run(init)"#JACOB

      print "Creating batch off of cat"#Jacob
      assert cat.shape == (224, 224, 3)
      batch = cat.reshape((1, 224, 224, 3))
      assert batch.shape == (1, 224, 224, 3)
      assert (0 <= batch).all() and (batch <= 1.0).all()
      print "\'Bout to run session(feed dictionary batch)"
      ...
      print "Stuffin it here"#JACOB
      top1 = print_prob(tf_activations['prob'])
      assert top1 == "n02123045 tabby, tabby cat"
    print "end tensorflow session"
  ...
  # in both networks.
  if ran_both:
    print "Now checking the product of both!"
